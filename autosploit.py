...
def exploit(query):
    ...
    print("[" + t.green("+") + \
        "] Sorting modules relevant to the specified platform.")
...
def settings():
    ...
    if workspace == False or local_host == False or local_port == False:
        configured = None
        print("\n[" + t.red("!") + \
            "] Warning. LPORT, LHOST and/or workspace cannot be null")
        ...
        if not os.path.isfile("hosts.txt"):
            print("[" + t.red("!") + \
                "] Warning. AutoSploit failed to detect host file.")
...
if __name__ == "__main__":
    ...
    if "Active: inactive" in postgresql:
        print("\n[" + t.red("!") + \
            "] Warning. Heuristics indicate Postgresql Service is offline")
        ...
        if start_pst == 'y':
            ...
            print("\n[" + t.red(
                "!") + "] AutoSploit's MSF related operations require this service to be active.")
            print("[" + t.red("!") + "] Aborted.")
            time.sleep(1.5)
            sys.exit(0)
        else:
